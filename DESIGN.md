...
graph TD
    ...
    subgraph core_engine ["Core Engine (app/)"]
        Events[(sample_events.jsonl <br> Stream Buffer)]
...
Events are emitted in JSONL format to `data/sample_events.jsonl` during processing and batched (50 events per batch) to the API's ingest endpoint in real time.
...
* **Store 2 Data Limitations:** Store 2 (ST1076 — Mumbai) only has video feeds and tracking event data (`sample_events_store2.jsonl`). Because no Point-of-Sale transaction data is loaded for this store, metrics such as conversion rate and purchase funnel stages will calculate as zero.
